...
import 'review_details_request.dart';
import 'review_details_response.dart';

enum ApiStatus { loading, empty, error, success }

class ReviewDetailsController extends GetxController {
  var myReviewDetails = <ReviewDetailsResponse>[].obs;
  ...
  fetchReviewDetailsFromApi({required listID}) async {
    try {
      int userId = await General.getPrefInt(ConstantValues.ID) ?? -1;
      ReviewDetailsRequest request = ReviewDetailsRequest(
          semesterId: semesterID.value, classId: classID.value,
          listId:listID, studentId: userId);
      ApiResponse? response = await HttpWrapper(
        context: Get.context,
        url: ApiEndPoint.REVIEWS_DETAILS_URL,
        body: request.toJson(),
      ).post();
      if (response!.statusCode == 200) {
        if (response.body != null) {
          ...
          myReviewDetails.value =
              data.map((item) => ReviewDetailsResponse.fromJson(item))
                  .toList();
        } else {
          status.value = ApiStatus.empty;
          Get.snackbar('Error', 'An error occurred while fetching review details');
        }
      } else {
        status.value = ApiStatus.error;
      }
    } catch (e) {
      showToast("Something went wrong while fetching review details: $e",
          backgroundColor: Colors.red, duration: const Duration(seconds: 3));
    }
  }
